 ...
 */
@Slf4j
public class WelcomeHandler extends AbstractStateNodeCallEventHandler{
    public WelcomeHandler(CallSessionManager callSessionManager) {
    ...
    protected void handleCallEvent(CallEvent event) {
        ...
        if(session != null) {
            ...
            log.info("播放系统欢迎语音:{} {}", event, session);
        } else {
            log.warn("来电丢失:{}", event);
        }
    }
